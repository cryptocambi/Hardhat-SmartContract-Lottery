...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle unit test", function () {
          ...
          describe("enterRaffle", function () {
              ...
              it("doesn't allow entrance when raffle is calculatin", async () => {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
          ...
          describe("checkUpkeep", function () {
              it("returns false if ppl haven't sent any ETH", async () => {
                  //await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
          ...
          describe("performUpkeep", function () {
              ...
              it("update the raffle state, emits an event, and calls the vrf coordiantor", async () => {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
          ...
          describe("FullfilrandomWord", async () => {
              beforeEach(async () => {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
